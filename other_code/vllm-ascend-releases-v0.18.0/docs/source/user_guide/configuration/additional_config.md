...
| `decode_full_attention_steps` | int | `1` | Number of initial decode metadata builds per request that keep the original full context before H2O pruning starts. The default protects TTFT by avoiding H2O compact metadata and attention graph updates on the first decode token; set this to `0` only for latency experiments that prove first-token H2O compaction is faster on the target model and graph mode. |
...
| `auto_tune_decode_warmup_steps` | int | `0` | Extra number of initial long-context decode metadata builds that keep the original full context before automatic H2O compaction starts. Effective warmup is the maximum of this value and `decode_full_attention_steps`, so the default still keeps the first decode token full-width for TTFT protection. |
...
For mixed 10k, 20k, 32k, and longer input / 1k output service benchmarks, the default automatic policy is the recommended starting point. It caps long-context decode metadata to a stable width after the first decode token, lifts the default ratio budget to that capped target for quality, keeps graph-capture and first-decode metadata full-width to avoid first-token setup overhead, and avoids having to retune `heavy_blocks`, `recent_blocks`, or `max_blocks` for each prompt length:
...
vllm serve /path/to/model \
  ...
  --max-num-seqs 32 \
  --block-size 128 \
  --additional-config='{"h2o_config":{"enabled":true,"min_seq_len":4096,"auto_tune":true,"auto_tune_max_blocks":64,"auto_tune_decode_warmup_steps":0,"selection_refresh_interval":128,"debug_log":false,"debug_timing":false,"debug_timing_sync":false}}' \
  --compilation-config '{"cudagraph_mode": "FULL_DECODE_ONLY", "cudagraph_capture_sizes": [1,2,4,8,12,16,32,64]}'
```

For mixed 10k, 20k, 32k, and longer input / 1k output, batch-size 32 service benchmarks, prefer the Ascend page-attention block size of 128 to reduce per-request block-table length before applying H2O. H2O is decode-only, so TTFT should stay close to the prefill baseline while TPOT and long-output E2E improve. Use the capped fast profile below when H2O should remain active for every prompt length without adding first-token setup overhead: it keeps graph-capture and first-decode metadata full-width, starts compact H2O from the second decode metadata build, keeps a 32-block short-context floor, scales 20k and 30k prompts to smaller active KV windows, caps very long prompts at 64 selected blocks, pads compact block-table metadata to a stable 64-column width to avoid length-specific decode graph updates after warmup, keeps stronger sink and recent budgets for quality, and avoids `max_prune_seq_len` so 20k, 32k, and max-model-len-permitted 100k prompts still use compact H2O metadata. This profile is latency-first after TTFT: on a 20k-token single request it can retain roughly 5k tokens after the first decode token, so use the conservative quality-validation profile below before judging long-context answer quality. The small metadata shape guard prevents 120-request continuous batches from mixing one full-context row with many pruned rows into a full-width gathered block table while preserving the intentional 64-column padding used by the 10k fast path.
...
vllm serve /path/to/model \
  ...
  --max-num-seqs 32 \
  --block-size 128 \
  --additional-config='{"h2o_config":{"enabled":true,"auto_tune":true,"auto_tune_max_blocks":64,"auto_tune_decode_warmup_steps":0,"heavy_blocks":24,"recent_blocks":24,"max_blocks":32,"min_seq_len":4096,"adaptive_min_keep_ratio":0.0,"adaptive_precision_ratio":0.0,"adaptive_precision_max_blocks":null,"min_prune_ratio":0.50,"min_metadata_prune_ratio":0.05,"history_cluster_size":2,"sink_blocks":8,"anchor_ratio":0.25,"score_explore_ratio":0.25,"score_coverage_ratio":0.50,"decode_full_attention_steps":1,"decode_budget_fast_blocks":32,"decode_budget_fast_ratio":0.25,"decode_budget_fast_max_blocks":64,"decode_budget_taper_steps":0,"decode_budget_taper_start_step":0,"selection_refresh_interval":128,"score_update_on_cache_hit":false,"debug_log":false,"debug_timing":false,"debug_timing_sync":false}}' \
  --compilation-config '{"cudagraph_mode": "FULL_DECODE_ONLY", "cudagraph_capture_sizes": [1,2,4,8,12,16,32,64]}'
```

For a short 20k/32k latency diagnosis, keep the same service command but enable timing summaries and use a sparse interval. The white-box logs include metadata build phases (`split_decodes_prefills`, `attention_mask`, `query_start_loc_h2d`, `metadata_object`, `h2o_apply`), H2O pruner phases (`prepare`, `plan`, `guard`, `select`, `score_update`, `build_metadata`), graph-capture metadata preparation, and attention graph update sub-phases (`workspace_ms`, `graph_update_ms`). Use these fields to determine whether TTFT is dominated by Python metadata, H2O block selection/gather, graph-capture preparation, workspace calculation, or per-layer graph update.
