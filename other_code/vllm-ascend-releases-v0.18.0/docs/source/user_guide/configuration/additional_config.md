...
| `decode_full_attention_steps` | int | `0` | Number of initial decode metadata builds per request that keep the original full context before H2O pruning starts. Keep this at `0` for TTFT-sensitive service benchmarks so H2O compacts the first decode metadata build; set it above `0` only when early-token quality is more important than first-token latency. |
