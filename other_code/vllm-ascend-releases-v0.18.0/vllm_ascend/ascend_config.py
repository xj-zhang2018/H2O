...
class AscendConfig:
    ...
    def __init__(self, vllm_config: "VllmConfig"):
        ...
        h2o_config = additional_config.get("h2o_config", {})
        self.h2o_config = H2OConfig(h2o_config)
...
class H2OConfig:
    """
    Configuration Object for block-level H2O KV-cache pruning.

    The Ascend fused attention kernels consume paged KV cache through block
    tables, so this config controls a block-granularity H2O policy for decode.
    It keeps a heavy-hitter block budget plus a recent block budget.
    """

    def __init__(self, h2o_config: dict | None = None):
        if h2o_config is None:
            h2o_config = {}
        if not isinstance(h2o_config, dict):
            raise TypeError("h2o_config must be a dict")

        self.enabled = bool(h2o_config.get("enabled", False))
        self.heavy_ratio = float(h2o_config.get("heavy_ratio", 0.1))
        self.recent_ratio = float(h2o_config.get("recent_ratio", 0.1))
        self.heavy_blocks = h2o_config.get("heavy_blocks", None)
        ...
        self.adaptive_precision_ratio = float(h2o_config.get("adaptive_precision_ratio", 0.6))
        self.adaptive_precision_max_blocks = h2o_config.get("adaptive_precision_max_blocks", 96)
        ...
        self.auto_tune_decode_warmup_steps = int(h2o_config.get("auto_tune_decode_warmup_steps", 32))
        ...
        self.score_update_on_cache_hit = bool(h2o_config.get("score_update_on_cache_hit", False))
    ...
    @staticmethod
    def _validate_optional_blocks(name: str, value):
        if value is None:
            return
        if not isinstance(value, int):
            raise TypeError(f"h2o_config.{name} must be an integer or None")
        if value < 0:
            raise ValueError(f"h2o_config.{name} must be greater than or equal to 0")

    def _validate(self):
        if self.heavy_ratio < 0 or self.recent_ratio < 0:
            raise ValueError("h2o_config heavy_ratio and recent_ratio must be non-negative")
        ...
        self._validate_optional_blocks("auto_tune_max_blocks", self.auto_tune_max_blocks)
        ...
        if self.auto_tune_max_blocks == 0:
            raise ValueError("h2o_config.auto_tune_max_blocks must be greater than 0 when set")
        if (
        ...
        if not 0 <= self.adaptive_precision_ratio <= 1.0:
            raise ValueError("h2o_config.adaptive_precision_ratio must be in the range [0, 1]")
        ...
        if self.history_cluster_size <= 0:
            raise ValueError("h2o_config.history_cluster_size must be greater than 0")
        ...
        heavy_budget_enabled = (
            (self.heavy_blocks is None and self.heavy_ratio > 0)
            or (self.heavy_blocks is not None and self.heavy_blocks > 0)
        )
        recent_budget_enabled = (
            (self.recent_blocks is None and self.recent_ratio > 0)
            or (self.recent_blocks is not None and self.recent_blocks > 0)
        )
        if not heavy_budget_enabled and not recent_budget_enabled:
            raise ValueError("h2o_config needs a positive heavy or recent budget when enabled")
