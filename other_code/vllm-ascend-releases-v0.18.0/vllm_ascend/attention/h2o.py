...
class H2OBlockPruner:
    ...
    def __init__(self):
        ...
        self._gather_indices_cache: tuple[tuple[Any, ...], torch.Tensor] | None = None
        ...
        self._seq_lens_incr_count: int = 0
    ...
    def _build_compact_metadata(
        ...
        gather_cache_key = (device_str, batch_size, block_tables.shape[1], metadata_width, selected_rows)
        gather_cached = self._gather_indices_cache
        if gather_cached is not None and gather_cached[0] == gather_cache_key:
            gather_indices = gather_cached[1]
            if _H2O_PROF_LOG:
                logger.info("[H2O-PROF] gather_indices: cache_hit 0ms")
        else:
            ...
            if _H2O_PROF_LOG:
                logger.info("[H2O-PROF] gather_buffer copy_: %.3fms",
                            (time.perf_counter() - _t_npu_copy) * 1000)
            self._gather_indices_cache = (gather_cache_key, gather_indices)
            self._seq_lens_incr_count = 0

        _t_gather_op = time.perf_counter() if _H2O_PROF_LOG else 0.0
        new_block_tables = block_tables.gather(1, gather_indices)
        if _H2O_PROF_LOG:
            logger.info("[H2O-PROF] block_tables.gather: %.3fms",
                        (time.perf_counter() - _t_gather_op) * 1000)
        ...
        if self._seq_lens_incr_count == 0:
            cpu_seq = torch.tensor(compact_seq_lens_list, dtype=seq_lens.dtype)
            self._seq_lens_buffer.copy_(cpu_seq)
            self._seq_lens_incr_count = 1
            if _H2O_PROF_LOG:
                logger.info("[H2O-PROF] seq_lens copy_ (initial): %.3fms",
                            (time.perf_counter() - _t_seq_lens) * 1000)
        else:
            self._seq_lens_buffer += 1
            self._seq_lens_incr_count += 1
            if _H2O_PROF_LOG:
                logger.info("[H2O-PROF] seq_lens += 1 (in-place): %.3fms, incr_count=%d",
                            (time.perf_counter() - _t_seq_lens) * 1000, self._seq_lens_incr_count)
        new_seq_lens = self._seq_lens_buffer

        prev_metadata_width = self._last_metadata_width
        width_changed = (
            prev_metadata_width is not None
            and prev_metadata_width != metadata_width
        )
        self._last_metadata_width = metadata_width
        if width_changed and _H2O_PROF_LOG:
            logger.info("[H2O-PROF] WARNING: metadata_width changed from %d to %d "
                        "- may trigger ACLGraph recapture",
                        prev_metadata_width, metadata_width)
